...
export function getScriptSource({ scriptId }) {
    ...
    // if (!script.getAttribute('src').startsWith("coui")) {
        // TODO this doesnt feel right, but didnt find something better to do here
        var request = new XMLHttpRequest()
        console.log("GET " + script.getAttribute('src'))
        request.open('GET', script.getAttribute('src'), false)  // `false` makes the request synchronous
        console.log("SEND")
        request.send()
        console.log("STATUS " + String(request.status))
        if (request.status === 200 || request.status === 0) {
            return { scriptSource: request.responseText }
        }
    // }
