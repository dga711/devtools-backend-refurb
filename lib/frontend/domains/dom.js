...
export function setAttributesAsText({ nodeId, name, text }) {
    ...
    Array.from(fakeElem.childNodes[0].attributes).forEach(
...
export function getBoxModel({ nodeId, backendNodeId, objectId }) {
    // const node = ObjectStore.getByObjectId(objectId)
    const root = Node.getNode(nodeId).node 

    if(!root.getBoundingClientRect){
        console.warn(root.nodeName + " has no getBoundingClientRect");
        return {};
    }

    // TODO this is giving the same rect for padding and margin too
    return {
        content: DOMQuad.fromRect(root.getBoundingClientRect()),
        padding: DOMQuad.fromRect(root.getBoundingClientRect()),
        margin: DOMQuad.fromRect(root.getBoundingClientRect()),
